...
const existingBlogPosts: any[] = [
    {
        ...
        category: 'AI',
        tags: [{ tag: 'deep-learning' }, { tag: 'conference' }, { tag: 'ai' }],
        excerpt:
    ...
    },
    {
        ...
        category: 'Tech',
        tags: [{ tag: 'education' }, { tag: 'scholarships' }, { tag: 'students' }, { tag: 'career' }],
        excerpt:
    ...
    try {
        ...
        if (!authorId) {
            return NextResponse.json(
                {
                    error: 'No users found. Please create an admin user first via /admin',
                },
                { status: 400 }
            )
        }
        ...
        for (const postData of existingBlogPosts) {
            ...
            if (existingPosts.docs.length > 0) {
                results.push({ slug: postData.slug, status: 'skipped', message: 'Already exists' })
                continue
            }
            ...
            results.push({ slug: postData.slug, status: 'created', id: newPost.id })
        }

        const published = results.filter((r) => r.status === 'created').length
        const skipped = results.filter((r) => r.status === 'skipped').length

        return NextResponse.json({
            success: true,
            message: 'Blog post seeding completed successfully',
            summary: {
                total: existingBlogPosts.length,
                created: published,
                skipped: skipped,
            },
            results,
